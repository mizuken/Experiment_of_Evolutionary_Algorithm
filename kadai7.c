...
//ここからアントコロニー最適化法
	...
	first_pheromone =(double)dimension/Lm;
	//フェロモンの初期化
	for(i = 0; i < dimension; i++){
		for(j = 0; j < dimension; j++){
	...
	while(loop_count != 1000){
		...
		for(i = 0; i < dimension; i++){
			for(j = i; j < dimension; j++){
				...
				temp_pheromone = 0;
				for(k = 0; k < ant; k++){
					if(ant_root_check[k][i][j] == 1 || ant_root_check[k][j][i] == 1){
						temp_pheromone += ant_root_reci_distance[k];
					}else{
						temp_pheromone += 0.0;
					}
				}
				pheromone[i][j] = (1.0 - GANMA)*pheromone[i][j] + temp_pheromone; //フェロモンの更新;
				pheromone[j][i] = (1.0 - GANMA)*pheromone[i][j] + temp_pheromone; //フェロモンの更新;
			}
		}
